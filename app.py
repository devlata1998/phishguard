...
def predict():
    try:
        ...
        save_flag = bool(data.get("save_to_db", False))

        if not subject and not body and not sender:
            return jsonify({"error": "Provide subject or body (or sender)"}), 400

        final_label, probs = backend.classify_email(
            subject, body, weights=weights, save_to_db=save_flag
        )
...
@app.route("/fetch/<label>", methods=["GET"])
def fetch(label):
    ...
    label_map = {
        ...
        "notspam": "not_phishing",
        "not-spam": "not_phishing",
    }
    target = label_map.get(label.lower(), label.lower())
    try:
        rows = backend.fetch_emails_by_label(target)
        return jsonify(rows)
    except Exception as e:
        traceback.print_exc()
        return jsonify({"error": str(e)}), 500
